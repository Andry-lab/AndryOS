...
PSF1_Font* LoadPSF1Font(EFI_FILE* Directory, CHAR16* Path, EFI_HANDLE ImageHandle, EFI_SYSTEM_TABLE* SystemTable) {
	...
	Systemtable->BootSerivices-> AllocatePool(EfiLoaderData, sizeof(PSF1_HEADER), (void**)&fontHeader);
	...
	UINT glyphBufferSize = fontHeader->charsize * 256;
 ...
 Framebuffer* initalizeGOP(){
	...
	if(EFI_ERROR(status)){
		Print(L"Impossibile Avviare Il server GOP(per La grafica)\n\r");
		return NULL;
	}
	else
	{
		Print(L"Avvio Del servizio GOP...\n\r");
		Print(L"Avvio Del Servizio GOP sucesso!!\n\r");
	}
